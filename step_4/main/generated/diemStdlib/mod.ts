...
export class ScriptFunctionCallVariantMintToken extends ScriptFunctionCall {
...
export class Stdlib {
   ...
   */
  static encodeInitializeErc20ScriptFunction(): DiemTypes.TransactionPayload {
    const tyArgs: Seq<DiemTypes.TypeTag> = [];
   ...
   */
  static encodeMintTokenScriptFunction(amount: bigint): DiemTypes.TransactionPayload {
    const tyArgs: Seq<DiemTypes.TypeTag> = [];
    ...
    const module_id: DiemTypes.ModuleId = new DiemTypes.ModuleId(new DiemTypes.AccountAddress([[36], [22], [58], [252], [198], [227], [59], [10], [148], [115], [133], [46], [24], [50], [127], [169]]), new DiemTypes.Identifier("ERC20"));
    const function_name: DiemTypes.Identifier = new DiemTypes.Identifier("mint_token");
    const script = new DiemTypes.ScriptFunction(module_id, function_name, tyArgs, args);
    return new DiemTypes.TransactionPayloadVariantScriptFunction(script);
  }
  ...
  static decodeInitializeErc20ScriptFunction(_script_fun: DiemTypes.TransactionPayload): ScriptFunctionCallVariantInitializeErc20 {
  ...
  static decodeMintTokenScriptFunction(script_fun: DiemTypes.TransactionPayload): ScriptFunctionCallVariantMintToken {
  if (script_fun instanceof DiemTypes.TransactionPayloadVariantScriptFunction) {
      ...
      return new ScriptFunctionCallVariantMintToken(
        amount
      );
    } else {
      throw new Error("Transaction payload not a script function payload")
  ...
  static ScriptFunctionArgs: {[name: string]: ScriptFunctionDef} = {
      ...
      args: [
                ...
                MintToken: {
      stdlibEncodeFunction: Stdlib.encodeMintTokenScriptFunction,
      description: "",
      typeArgs: [],
      args: [
